...
func parseSchemaSource(p schemaSourceFlags) (schemaSourceFactory, error) {
	...
	if p.targetDatabaseDSN != "" {
		cfg, err := mysql.ParseDSN(p.targetDatabaseDSN)
		if err != nil {
			return nil, fmt.Errorf("parsing DSN %q: %w", p.targetDatabaseDSN, err)
		}
		return func() (diff.SchemaSource, io.Closer, error) {
			connPool, err := newMySQL(cfg.FormatDSN())
			if err != nil {
...
func generatePlan(ctx context.Context, logger log.Logger, connCfg *mysql.Config, planConfig planConfig) (diff.Plan, error) {
	tempDbFactory, err := tempdb.NewOnInstanceFactory(ctx, func(ctx context.Context, dbName string) (*sql.DB, error) {
		copiedConfig := connCfg.Clone()
		copiedConfig.DBName = dbName
		return newMySQL(copiedConfig.FormatDSN())
	}, tempdb.WithRootDatabase(connCfg.DBName))
	if err != nil {
	...
	connPool, err := newMySQL(connCfg.FormatDSN())
	if err != nil {
		return diff.Plan{}, err
	}
